...
mydb=mysql.connector.connect(user='root',host='localhost',password='Eswar@123',database='snm_db')

app=Flask(__name__)
app.config['SESSION_TYPE']='filesystem'
Session(app)
app.secret_key='code@1999'
    ...
    if session.get('user'):
        if request.method=='POST': 
            ...
            try:
                ...
                if user_id:
                    cursor.execute('insert into notes(n_title,n_content,added_by) values(%s,%s,%s)', [title,description,user_id[0]])
                    mydb.commit()
                    cursor.close()
                else:
...
def viewallnotes():
    if session.get('user'):
        try:
            ...
            user_id=cursor.fetchone()
            if user_id:
                cursor.execute('select notesid,n_title,created_at from notes where added_by=%s',[user_id[0]])
...
def getexceldata():
   if session.get('user'):
      try:
         ...
         if user_id:
            cursor.execute(
                'select notesid,n_title,n_content,created_at from notes where added_by=%s',
                [user_id[0]]
            )
            allnotes_data=cursor.fetchall()
            cursor.close()
        ...
        if allnotes_data:
            ...
            heading = ['Notes_id','Title','content','Created_time']
        ...
        try:
            ...
            if request.method=='POST':
                ...
                try:
                    ...
                    user_id=cursor.fetchone() #(1,)
                    if user_id:
                        cursor.execute('update notes set n_title=%s,n_content=%s where added_by=%s and notesid=%s',[updated_title,updated_desc,user_id[0],nid])
                        mydb.commit()
                        cursor.close()
                    else:
                        flash('could not get the user data to fetch notes')
    ...
    if session.get('user'):
        ...
        if pattern.match(search_data):
            try:
                ...
                user_id=cursor.fetchone() #(1,)
                if user_id:
                    cursor.execute('select  notesid,n_title,created_at from notes where added_by=%s and n_title like %s',[user_id[0],search_data+'%'])
